...
public:
	...
	rt::ray_bounce get_bounce(const path_tracer &ctx, const ray_hit &hit, float ior) const override
	{
		...
		bounce.emission = 0.25f * glm::mix(glm::vec3{0.3, 0.3, 0.4}, glm::vec3{1.1, 1.0, 2.0}, std::abs(hit.direction.y));
		return bounce;
	}
};
